 ...
 * 代码清单9-7 聊天室服务端程序
 */
#define _GNU_SOURCE 1  // 为了使用 POLLRDHUP
#include <sys/types.h>
...
int main ( int argc, char* argv[] ) {
     ...
     * 并且 socket 的值可以直接用来索引（作为数组下标）socket 连接对应的 client_data对象，
    ...
    while ( true ) {
        ...
        for ( int i = 0; i < user_counter+1; ++i ) {
            ...
            if ( ( fds[i].fd == listenfd ) && ( fds[i].revents & POLLIN ) ) {
                ...
                /* 对于新的连接，同时修改 fds 和 users 数组。users[connfd]对应新连接文件描述符connfd的客户数据 */
                user_counter++;
            ...
            /* connfd 上的事件 */
                ...
                users[fds[i].fd] = users[fds[user_counter].fd];  // 把最后一个user放到要关闭连接用户的位置上去
                close ( fds[i].fd );
